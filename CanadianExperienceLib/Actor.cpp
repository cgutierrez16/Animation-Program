...
void Actor::SetPicture(Picture *picture)
{
    ...
    // Get the timeline from the picture
    Timeline *timeline = mPicture->GetTimeline();
    // Add the actor's channel to the timeline
...
/**
 * Deletes keyframe in timeline
 * @param frame Frame of animation to delete keyframe
 */
void Actor::DeleteKeyframe(int frame)
{
    for (auto drawable : mDrawablesInOrder)
    {
        drawable->DeleteKeyframe(frame);
        mChannel.DeleteKeyframe(frame);
    }
}
...
void Actor::GetKeyframe()
{
    if (mChannel.IsValid())
    {
        wxPoint pos = mChannel.GetPosition();
        mPosition = pos;
    }
