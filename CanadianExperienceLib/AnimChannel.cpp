 ...
 */
void AnimChannel::DeleteKeyframe(int frame)
{
    auto iter = std::find_if(mKeyframes.begin(), mKeyframes.end(),
        [frame](auto& keyframe) { return keyframe->GetFrame() == frame;});
    if (iter != mKeyframes.end())
    {
        int index = std::distance(mKeyframes.begin(), iter);
        ...
        SetFrame(mTimeline->GetCurrentFrame());
    }
}